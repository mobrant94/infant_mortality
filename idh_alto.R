...
corrplot(matriz_cor, method="color", type="lower", 
         ...
         col=colorRampPalette(c("blue", "white", "red"))(200), # Paleta de cores
...
modelo_glm7 <- glm.nb(rate ~ spline1*bcg_d+spline2*bcg_d+spline3*bcg_d,
                      data = esgoto %>% filter(bcg>=1), weights = populacao)
summary(modelo_glm7)
...
# Exportar para Excel
write_xlsx(resultados_todos, "resultados_modelos_novo.xlsx")
...
g5y = ggplot(df_plot, aes(x = ano, y = rate)) +
...
models <- list(
  "APS" = glm.nb(rate ~ gasto_aps, data = im_data %>% filter(gasto_aps >= 0), weights = populacao),
  "Água" = glm.nb(rate ~ investimento_agua_prestador, data = im_data %>% filter(investimento_agua_prestador >= 0), weights = populacao),
  "Esgoto" = glm.nb(rate ~ investimento_esgoto_prestador, data = im_data %>% filter(investimento_esgoto_prestador >= 0), weights = populacao),
  "Bolsa Família" = glm.nb(rate ~ valor_repassado_bolsa_familia_def, data = im_data %>% filter(valor_repassado_bolsa_familia_def >= 1), weights = populacao)
)

# Modelos com interação (proporção de favela)
models_interaction <- list(
  "APS x Favela" = glm.nb(rate ~ gasto_aps:proporcao_favela, data = im_data %>% filter(gasto_aps >= 0), weights = populacao),
  "Água x Favela" = glm.nb(rate ~ investimento_agua_prestador:proporcao_favela, data = im_data %>% filter(investimento_agua_prestador >= 0), weights = populacao),
  "Esgoto x Favela" = glm.nb(rate ~ investimento_esgoto_prestador:proporcao_favela, data = im_data %>% filter(investimento_esgoto_prestador >= 0), weights = populacao),
  "Bolsa Família x Favela" = glm.nb(rate ~ valor_repassado_bolsa_familia_def:proporcao_favela, data = im_data %>% filter(valor_repassado_bolsa_familia_def >= 1), weights = populacao)
)
...
write_xlsx(results, "resultados_modelos.xlsx")
