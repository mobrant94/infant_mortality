...
# Exportar para Excel
write_xlsx(resultados_todos, "resultados_modelos_novo1a5a.xlsx")
...
g15a = ggplot(df_plot, aes(x = ano, y = rate)) +
...
# Criar a composição dos gráficos removendo rótulos duplicados
(g5y + g28d + g28d1a + g15a) +
  plot_layout(axis = "collect") # Coleta os eixos em comum
...
modelo_poisson <- glm(rate ~ investimento_esgoto_prestador, 
                      data = im_data %>% filter(investimento_esgoto_prestador >= 0), 
                      family = poisson(link = "log"))

# Modelos sem interação
models <- list(
  "APS" = glm.nb(rate ~ gasto_aps, data = im_data %>% filter(gasto_aps >= 0), init.theta = 0.1),
  "Água" = glm.nb(rate ~ investimento_agua_prestador, data = im_data %>% filter(investimento_agua_prestador >= 0.1), init.theta = 0.1),
  "Bolsa Família" = glm.nb(rate ~ valor_repassado_bolsa_familia_def, data = im_data %>% filter(valor_repassado_bolsa_familia_def >= 1), init.theta = 0.1)
)
...
write_xlsx(results, "resultados_modelos1a5a.xlsx")
